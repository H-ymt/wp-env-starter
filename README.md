...
- [目次](#目次)
- [🔗 参考リンク](#-参考リンク)
- [🖥️ WP環境について](#️-wp環境について)
  - [動作環境](#動作環境)
- [⚙️ 動作環境](#️-動作環境)
- [🏷️ WordPress環境タイプ](#️-wordpress環境タイプ)
- [🏗️ ローカル環境セットアップ手順](#️-ローカル環境セットアップ手順)
  - [よく使うコマンド](#よく使うコマンド)
    - [wp:setup](#wpsetup)
    - [wp:stop](#wpstop)
- [🚀 本番環境へのアップロード](#-本番環境へのアップロード)
- [🔄 ブラウザ同期（Browser Sync）](#-ブラウザ同期browser-sync)
- [🖼️ CSSから画像を参照する方法](#️-cssから画像を参照する方法)
- [📦 アセットの扱い方](#-アセットの扱い方)
- [🧹 Lintについて](#-lintについて)
- [📚 参考ドキュメント](#-参考ドキュメント)
- [🆘 トラブルシュート](#-トラブルシュート)
...
- WordPress環境の起動
- DBインポート（`sql/wpenv.sql.gz` → 展開 → インポート）
- リライトルールの再構築
...
- `npm run wp:setup` : WordPress環境の起動・初期セットアップ（DBインポート・リライトルール再構築）
- `npm run wp:stop` : WordPress環境の停止とDBエクスポート（バックアップ）

#### wp:setup

WordPress環境の起動・初期セットアップを一括で行います。

1. WordPress環境の起動
2. DBインポート（`sql/wpenv.sql.gz` → 展開 → インポート）
3. リライトルールの再構築

#### wp:stop

WordPress環境の停止とDBのエクスポート（バックアップ）を行います。

1. DBエクスポート（`sql/wpenv.sql` → gzip圧縮）
2. WordPress環境の停止
...
- Refs: https://github.com/liginc/wp-starter-theme
